...
const dashboardButtons = require('./dashboardButtons');
...
class ButtonManager {
    constructor(client) {
        ...
        this.registerHandler(['refresh_dashboard', 'bot_settings', 'view_streamers'], dashboardButtons);
    }
    ...
    async handleInteraction(interaction) {
        ...
        for (const [prefix, handler] of this.handlers) {
            if (interaction.customId === prefix || interaction.customId.startsWith(prefix)) {
                try {
                    return await handler.handle(interaction, this.client);
                } catch (error) {
